...
@app.route('/originalComment', methods=['GET'])
def create_Comment():
    response = client.chat.completions.create(
        model="gpt-4o-mini", messages=[{"role": "system", "content": "you are a customer who bought one of the products provided as a context:{products} and your task is to write either a positive or negative comment on any one of the products, include negative comments to have varied response every time "}, {"role": "user", "content": "write either a 100 word positive or negative comment on one of the products. and include any user name as the writter"}])
    final_response = response.choices[0].message.content
    return (jsonify({"messages": final_response}))
...
def compose_email():
    ...
    subject = create_subject(comment)
    summary = summarize_summary(comment)
    sentiment = get_sentiment(comment)
    print(subject)
    print(summary)
    email = client.chat.completions.create(model="gpt-4o-mini", messages=[{
        "role": "system", "content": f""" You are a helpfull Ai assistant for an electronic shop that composes an email based on the below comment which is delimited by triple backticks and the subject provided below delimitted by dollar sign and the sentiment of the comment delimitted by hastags. comment:'''{summary}''', subject:${subject}$ , sentiment: #{sentiment}# and the output should be in the provided language below language :$${language}$$, and company information to be in the footer are the following : % name: Jeff Bezos, position: Manager ,contact: +1 925-22122-212, company name: Emax electronics % """
    }])
    final_email = email.choices[0].message.content
    return (jsonify({"email": final_email, "summary": summary, "sentiment": sentiment}))


def create_subject(comment):
    response = client.chat.completions.create(model="gpt-4o-mini", messages=[
                                              {"role": "system", "content": "You are a helpfull ai Assistant for an electronics shop that creates a potential subject for an email response to a users comment about a product"}, {"role": "user", "content": f"{comment}"}])
...
def get_sentiment(comment):
    response = client.chat.completions.create(model="gpt-4o-mini", messages=[
                                              {"role": "system", "content": "You are a helpfull ai Assistant for an electronics shop that extracts the sentiment of a user comment on a product. give a single word reponse as positive or negative"}, {"role": "user", "content": f"{comment}"}])
    email_subject = response.choices[0].message.content
    return email_subject


def summarize_summary(comment):
    summary = client.chat.completions.create(model="gpt-4o-mini", messages=[
                                             {"role": "system", "content": "You are a helpfull ai Assistant for an electronics shop that summarizes users comment on a product that they purchased from the shop"}, {"role": "user", "content": f"{comment}"}])
